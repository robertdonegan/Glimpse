...
export function PoseGizmo({ pose, onChange }: { pose: Pose; onChange: (p: Pose) => void }) {
  ...
  const start = (axis: 'x' | 'y' | 'z') => (e: React.PointerEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    const move = (ev: PointerEvent) => {
      ...
      if (axis === 'x') {
        acc.rotX = clamp(acc.rotX - dy * 0.4, -75, 75);
      } else if (axis === 'y') {
        acc.rotY = clamp(acc.rotY + dx * 0.4, -75, 75);
      } else {
        ...
        if (d < -180) d += 360;
        lastAng = ang;
        acc.rotZ = clamp(acc.rotZ + d, -20, 20);
      }
      onChange({
  ...
  return (
    ...
    >
      ...
      <ellipse cx={50} cy={50} rx={RX} ry={flatX} className="giz-ring giz-x" />
      ...
      <ellipse cx={50} cy={50} rx={flatY} ry={RY} className="giz-ring giz-y" />
      ...
      <g transform={`rotate(${pose.rotZ} 50 50)`}>
        ...
        <circle cx={50} cy={50} r={RZ} className="giz-ring giz-z" />
        <line x1={50} y1={50} x2={50} y2={50 - RZ} className="giz-z giz-tick" />
      </g>
